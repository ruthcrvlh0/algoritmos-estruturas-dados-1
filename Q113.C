...
void salvarFila(Fila *f, const char *nomeArquivo) {
    ...
    if (arquivo == NULL) {
        printf("\n>> Erro critico: Nao foi possivel criar o arquivo de backup!\n");
        return;
    }
    ...
    fclose(arquivo);
    printf("\n>> BACKUP REALIZADO: %d jogadores salvos em '%s'.\n", contador, nomeArquivo);
}
        ...
        switch (opcao) {
            ...
            case 6:
                if (estaVazia(&filaMatchmaking)) {
                    printf("\n>> AVISO: A fila esta vazia. Nada para salvar.\n");
                } else {
                    salvarFila(&filaMatchmaking, "matchmaking_queue.bin");
                }
                break;
