...
// Struct para representar o Mercenário
typedef struct {
    int id;
    char nome[50];
} Mercenario;
...
// --- FUNÇÃO FOCO: CONTAR O TAMANHO DA LISTA ---

/**
 * @brief Percorre a lista encadeada para contar o número total de nós.
 * @param lista Ponteiro para a estrutura Lista.
 * @return O número de elementos na lista (int).
 */
int tamanho(Lista *lista) {
    int contador = 0;
...
int main() {
    ...
    do {
        ...
        switch(opcao) {
            ...
            case 3:
                printf("\n>>> Censo da Guilda <<<\n");
                int num_mercenarios = tamanho(companhia);
                printf("A companhia possui atualmente %d mercenario(s) ativo(s).\n", num_mercenarios);
                break;
            case 4:
...
void menu() {
    ...
    printf("3. Realizar Censo (Verificar Tamanho)\n"); /
    printf("4. Sair\n");
